...
export function App() {
  ...
  const [emailsToInvite, setEmailsToInvite] = useState([
    'luana.andrade@rockeat.com.br',
    'giovanna@rockeat.com.br'
  ])
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <div className="h-16 bg-zinc-900 px-4 rounded-xl flex items-center shadow-shape gap-3">
            <button type="button" onClick={openGuestsModal} className="flex items-center gap-2 flex-1 text-left">
             ...
             <span className="text-zinc-488 text-lg flex-1">Quem estará na viagem?</span>
            </button>
      ...
      <div className="w-[640px] rounded-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {emailsToInvite.map(email => {
              return (
                    <div key={email} className="py-1.5 px-2.5 rounded-nd bg-zinc-800 flex items-center gap-2">
                        <span className="text-zinc-300">{email}</span>
                        <button className="text-zinc-400" onClick={() => removeEmailFromInvites(email)}>
